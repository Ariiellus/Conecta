...
export default function ConfirmationModal({
  ...
  const getCurrencySymbol = (currency: string) => {
    if (currency === "USD" || currency === "USDC") return "$";
    if (currency === "EURC") return "€";
    return "";
  };

  const currencyCode = transactionData.currency || "USD";
  const currencySymbol = getCurrencySymbol(currencyCode);
  
  const formattedAmount = transactionData.amount 
    ? `${currencySymbol}${Number(transactionData.amount).toFixed(2)} ${currencyCode}` 
    : `${currencySymbol}0.00 ${currencyCode}`;
